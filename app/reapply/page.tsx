...
const GENDERS = [
  'woman','man','non_binary','trans_woman','trans_man','genderqueer','genderfluid',
  'agender','other','prefer_not_to_say',
]
const MODALITIES = [
...
export default function ReapplyPage() {
  ...
  useEffect(() => {
    fetch('/api/companions/reapply')
      ...
      .then(data => {
        if (!data) { router.replace('/status'); return }
        setCity(data.city ?? '')
  ...
  async function handleSubmit() {
    ...
    try {
      const r = await fetch('/api/companions/reapply', {
        ...
        body: JSON.stringify({ city, gender, tagline, bio, session_modality: modality, whatsapp_number: whatsapp || undefined }),
      })
  ...
  const S: Record<string, React.CSSProperties> = {
    page: { minHeight: '100vh', background: '#07090f', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '32px 16px' },
    wrap: { width: '100%', maxWidth: 520 },
    logo: { fontFamily: 'var(--font-serif)', fontSize: 20, color: '#eeeef0', display: 'block', marginBottom: 32 },
    card: { background: '#0d1117', border: '1px solid #1c2333', borderRadius: 20, overflow: 'hidden', padding: '32px' },
    ...
    btn: { width: '100%', background: '#e8607a', color: '#fff', border: 'none', borderRadius: 12, padding: '14px', fontSize: 15, fontWeight: 500, cursor: 'pointer', minHeight: 48, marginTop: 8 },
  ...
  function pill(val: string, current: string, onClick: () => void, label?: string) {
    ...
    return (
      <button key={val} onClick={onClick} style={{
        padding: '6px 14px', borderRadius: 999, fontSize: 13, cursor: 'pointer', transition: 'all .15s', minHeight: 34,
    ...
    return (
      <div style={S.page}>
        <div style={S.wrap}>
          ...
          <div style={{ height: 200, background: '#111620', borderRadius: 16, animation: 'shimmer 1.5s infinite' }} />
        </div>
      </div>
    )
  }
    ...
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        <div style={S.card}>
          ...
          <h1 style={S.heading}>Let&rsquo;s try <em style={{ color: '#e8607a' }}>again.</em></h1>
          <p style={S.sub}>Update the details below and re-submit. We&rsquo;ll review your application again shortly.</p>

          {error && <div style={S.err}>{error}</div>}
          {success && <div style={{ ...S.err, color: '#22c55e', borderColor: 'rgba(34,197,94,.25)', background: 'rgba(34,197,94,.08)' }}>Submitted — redirecting…</div>}

          <label style={S.fieldLabel}>City</label>
          <input style={S.input} value={city} onChange={e => setCity(e.target.value)} placeholder="Amsterdam" />

          <label style={S.fieldLabel}>WhatsApp number (E.164 format)</label>
          <input style={S.input} value={whatsapp} onChange={e => setWhatsapp(e.target.value)} placeholder="+31612345678" inputMode="tel" />
          ...
          <textarea style={S.textarea} value={bio} onChange={e => setBio(e.target.value)} placeholder="Tell dreamers about yourself…" maxLength={2000} />

          <label style={S.fieldLabel}>Session type</label>
          <div style={S.pillRow}>
            {MODALITIES.map(m => pill(m.v, modality, () => setModality(m.v === modality ? '' : m.v), m.label))}
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 16 }}>
            <a href="/status" style={{ fontSize: 13, color: '#4b5563', textDecoration: 'none' }}>← Back to status</a>
          </div>
        </div>
      </div>
    </div>
