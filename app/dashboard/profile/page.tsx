...
const S = {
  ...
  title:        { fontFamily: 'var(--font-serif)', fontSize: 28, color: '#eeeef0', lineHeight: 1.2, marginBottom: 6 } as React.CSSProperties,
  sub:          { fontSize: 14, color: '#6b7280', marginBottom: 0 } as React.CSSProperties,
  card:         { background: '#0d1117', border: '1px solid #1c2333', borderRadius: 16, padding: '28px' } as React.CSSProperties,
  label:        { fontSize: 12, color: '#9ca3af', marginBottom: 6, display: 'block' } as React.CSSProperties,
  input:        { width: '100%', background: '#111620', border: '1px solid #1c2333', borderRadius: 12, padding: '12px 14px', fontSize: 15, color: '#eeeef0', outline: 'none', marginBottom: 16, boxSizing: 'border-box' } as React.CSSProperties,
  select:       { width: '100%', background: '#111620', border: '1px solid #1c2333', borderRadius: 12, padding: '12px 14px', fontSize: 15, color: '#eeeef0', outline: 'none', marginBottom: 16, cursor: 'pointer', appearance: 'none' as const, boxSizing: 'border-box' as const },
  textarea:     { width: '100%', background: '#111620', border: '1px solid #1c2333', borderRadius: 12, padding: '12px 14px', fontSize: 15, color: '#eeeef0', outline: 'none', resize: 'vertical' as const, minHeight: 120, marginBottom: 16, boxSizing: 'border-box' as const },
  grid2:        { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 } as React.CSSProperties,
  saveBtn:      { background: '#e8607a', color: '#fff', border: 'none', borderRadius: 12, padding: '13px 28px', fontSize: 14, fontWeight: 500, cursor: 'pointer', minHeight: 44 } as React.CSSProperties,
  successMsg:   { background: 'rgba(34,197,94,.08)', border: '1px solid rgba(34,197,94,.25)', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#22c55e', marginBottom: 16 } as React.CSSProperties,
  errMsg:       { background: 'rgba(248,113,113,.08)', border: '1px solid rgba(248,113,113,.25)', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#f87171', marginBottom: 16 } as React.CSSProperties,
  shimmer:      { background: 'linear-gradient(90deg,#111620 25%,#1c2333 50%,#111620 75%)', backgroundSize: '200% 100%', borderRadius: 16, animation: 'shimmer 1.5s infinite', height: 400 } as React.CSSProperties,
  sectionTitle: { fontFamily: 'var(--font-serif)', fontSize: 18, color: '#eeeef0', marginBottom: 16, marginTop: 8 } as React.CSSProperties,
...
function VerificationView({ me }: { me: MeData }) {
  ...
  const bannerStyle: React.CSSProperties = isRejected
    ? { background: 'rgba(248,113,113,.07)', border: '1px solid rgba(248,113,113,.25)', borderRadius: 14, padding: '18px 20px', marginBottom: 28 }
    : { background: 'rgba(251,191,36,.06)', border: '1px solid rgba(251,191,36,.2)', borderRadius: 14, padding: '18px 20px', marginBottom: 28 }
    ...
    <div style={S.page}>
      ...
      <div style={bannerStyle}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: isRejected ? '#f87171' : '#fbbf24', marginBottom: 4 }}>
            ...
            {isRejected && me.rejection_reason ? (
              ...
              <p style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.6, margin: 0 }}>
                {isPending
                  ? 'Our team reviews every application personally. We\'ll notify you by email at ' + me.email + '.'
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#6b7280', marginBottom: 4 } as React.CSSProperties}>
      ...
      <div style={S.card}>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input
            ...
            tabIndex={-1}
          />
          <p style={{ fontSize: 11, color: '#4b5563', marginTop: 4 }}>Email cannot be changed after verification.</p>
        </div>
        ...
        <div style={S.grid2}>
          <div>
            ...
            <input style={S.input} value={form.full_name} onChange={e => set('full_name', e.target.value)} placeholder="Your legal name" />
          </div>
          <div>
            <label style={S.label}>Display name</label>
            <input style={S.input} value={form.display_name} onChange={e => set('display_name', e.target.value)} placeholder="Name dreamers know you by" maxLength={60} />
          </div>
        </div>
        ...
        <div style={S.grid2}>
          <div>
            ...
            <input style={S.input} value={form.country} onChange={e => set('country', e.target.value)} placeholder="Netherlands" autoCapitalize="words" />
          </div>
          <div>
            <label style={S.label}>City</label>
            <input style={S.input} value={form.city} onChange={e => set('city', e.target.value)} placeholder="Amsterdam" autoCapitalize="words" />
          </div>
        </div>
        ...
        {/* Save */}
        <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 8, borderTop: '1px solid #1c2333', marginTop: 8 }}>
          <button
...
function ProfileBuilder() {
  ...
  useEffect(() => {
    fetch('/api/companions/profile')
      ...
      .then(d => { if (d) setProfile({ ...DEFAULT_PROFILE, ...d }) })
      ...
      <div style={S.card}>
        ...
        {active === 1 && (
          <>
            ...
            <textarea style={{ ...S.textarea, minHeight: 160 }} value={profile.bio ?? ''} onChange={e => set('bio', e.target.value)} placeholder="Tell dreamers about yourself…" maxLength={2000} />
        ...
        {active === 7 && (
          <>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.6, marginBottom: 20 }}>Set your base hourly rate.</p>
            <div style={S.grid2}>
              <div>
...
export default function ProfilePage() {
  ...
  if (me.status === 'approved') return <ProfileBuilder />
